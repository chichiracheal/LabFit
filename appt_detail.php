...
$appt=new User;
$a=$appt->get_all_appts($_SESSION['useronline']);
...
<div class="content mt-2" style="background-color: white;padding:3em">
  ...
  <?php
    if(is_array($a)){
      ...
      foreach($a as $appt){
 $tamt = number_format($appt['Test_amt'],2);
...
?>
      ...
      <tr>
        ...
        <td><?php print $appt['Test_name']?></td>
        <td><?php print "&#8358;$tamt";?></td>
        <td><?php print $appt['Appt_date'] ?></td>
        <td><?php print $appt['Appt_time'] ?></td>
        <td class=" btn btn-sm btn-dark"><?php print $appt['Appt_status'] ?></td>
        <td><?php print $appt['Appt_note'] ?></td>
        <td><?php print $appt['Appt_remark'] ?></td>
        <td>
        <a href="payroll.php?id=<?php print $appt['Appt_id']?>" id="link" class="btn btn-primary mb-2" id="btn">Pay</a>
      
        <a onclick="return confirm('Are you sure you want to delete?')"
        href="delete_appt.php?id=<?php print $appt['Appt_id'] ?>" class="btn btn-sm btn-danger">Cancel</a></td> 
