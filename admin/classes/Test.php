...
class Test extends Db{
    ...
    public function get_result_by_id($id){
        $sql = "SELECT * FROM `appointment` JOIN result ON Appt_id=Res_appt_id WHERE Appt_id=? ";
        $stmt = $this->dbconn->prepare($sql);
        $stmt->execute([$id]);
    ...
    public function get_all_payments(){
        $query = "SELECT * FROM payments JOIN patients ON Patient_Id=Pay_by";
        $stmt = $this->dbconn->prepare($query);
        $stmt->execute();
        $record = $stmt->fetchAll();
        return $record;
    ...
    public function get_all_trans(){
        $query = "SELECT * FROM transaction JOIN trans_details JOIN tests ON Patient_Id=Appt_by AND Test_id = Appt_test_id";
        $stmt = $this->dbconn->prepare($query);
        $stmt->execute();
        $record = $stmt->fetchAll();
        return $record;
    ...
    public function get_user_test(){
        $query = "SELECT * FROM test JOIN level ON topic_level=level_id WHERE topic_status=1";
        $stmt = $this->dbconn->prepare($query);
        $stmt->execute();
        $record = $stmt->fetchAll();
        return $record;
    ...
    public function update_appt($alltest,$adate,$time,$status,$msg,$id){
     ...
     //a method to update appt
     public function update_pay($status,$id){
        $sql = "UPDATE payments  SET Pay_status =? WHERE Pay_id =?";
        $stmt = $this->dbconn->prepare($sql);
    ...
    public function get_all_results($id){
     ...
     //to edit
     public function get_pay_by_id($id){
        $query = "SELECT * FROM payments WHERE Pay_id=?";
        $stmt = $this->dbconn->prepare($query);
        $stmt->execute([$id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result;
